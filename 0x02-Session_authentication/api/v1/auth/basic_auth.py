...
class BasicAuth(Auth):
    ...
    def current_user(self, request=None) -> TypeVar('User'):
        ...
        # Decode base64 part of authorization header
        decoded_base64_part = self.decode_base64_authorization_header(
            base64_part)
        if not decoded_base64_part:
            return

        # Get user email and password from decoded authorization header
        user_email, user_pwd = self.extract_user_credentials(
            decoded_base64_part)

        # Get user object from database using email and password
